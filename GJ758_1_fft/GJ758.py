...
route = r"F:\DirectImaging\data-201706\GJ758\GJ758_1"
...
side=2*100
print("The side of the image after registration is",side)
foo_shift=foo[int(215.8-side/2): int(215.8+side/2), int(280.2-side/2):int(280.2+side/2)]
fig = plt.figure(figsize = (12, 8))
plt.ion()
plt.subplot(121)
plt.imshow(foo, cmap= "jet")
...
try again
"""
centroid = Center_of_Gravity(foo_shift)
print ( "The ceneter of original image is:", centroid)
# do shifting here
side=2*(math.ceil(min(img_size[0]/2-abs((img_size[0]/2)-centroid[1]), img_size[1]-abs((img_size[1]/2)-centroid[0]))))
print("The side of the image after registration is",side)
...
plt.imshow(np.log10(np.add(1,np.multiply(1.0,F1_abs))), cmap="jet")
plt.colorbar()
plt.savefig("the original fourier transform image_4")
...
plt.imshow(A_2, cmap="jet")
plt.colorbar()
plt.savefig("the phase spectrum of the image_4")
plt.show()
